...
# EIIE Implementation
class Portfolio():
    def __init__(self, symbols, weights, pBeta, k, learningRate, minibatchCount, minibatchSize, epochs, noop=False,
                 failureChance=0):
    ...
    def createEiieNet(self, inputTensor, rates):
        ...
        # print("rates: ", rates)
        print("mainInputShape: ", mainInputShape)

        print("weightInputShape: ", weightInputShape)

        mIn = Input(shape=mainInputShape, name='mainInput')

        print("x after before convolution layer 1 and activation: ", mIn)
        x = Conv2D(2, (3, 1))(mIn)
        print("x before activation 1: ", x)
        x = Activation('relu')(x)
        print("x before convolution layer 2 : ", x)
        x = Conv2D(20, (48, 1))(x)
        print("x before activation 2: ", x)
        x = Activation('relu')(x)
        print("x after activation 2: ", x)
    ...
    def trainOnMinibatch(self, idx, inTensor, rates):
        ...
        mu = []
        for wPT, wT in zip(wPrime, squeezeOut if len(squeezeOut.shape) > 1 else [squeezeOut]):
            mu.append([self.calculateMu(wPT, wT, self.k)])

        mu = np.array(mu)

        grad = self.getGradient(inputs=[mIn, wIn, bIn, mu, yC, out])  # error here
    ...
    def calculateMu(self, wPrime, w, k):
        ...
        for i in range(k):
            muSuffix = np.sum(np.maximum(wPrime - mu * w, 0))
            mu = (1. / (1. - self.tradeFee * w[0])) * (
                        1. - (self.tradeFee * wPrime[0]) - (2 * self.tradeFee - (self.tradeFee ** 2)) * muSuffix)
        return mu
